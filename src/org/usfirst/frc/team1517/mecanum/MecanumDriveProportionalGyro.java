 ...
 *
 */
public class MecanumDriveProportionalGyro
{
	double p;
    ...
    public MecanumDriveProportionalGyro(SpeedController aFront, SpeedController aBack, SpeedController bFront, SpeedController bBack, double pCoefficient)
    {
        aF = aFront;
        aB = aBack;
        bF = bFront;
    ...
    public void slide(double magnetude)
    {
    	double correction = p * gyro.getAngle(), highestValue;
    	
    	if(Math.abs(magnetude + correction) > 1 || Math.abs(magnetude - correction) > 1 || Math.abs(-1 * magnetude + correction) > 1 || Math.abs(-1 * magnetude - correction) > 1)
        {
            highestValue = Math.abs(magnetude + correction);
            
            if(Math.abs(magnetude - correction) > highestValue)
            {
                highestValue = Math.abs(magnetude - correction);
            }
            
            if(Math.abs(-1 * magnetude + correction) > highestValue)
            {
                highestValue = Math.abs(-1 * magnetude + correction);
            }
    	...
    	System.out.println("Correction: " + correction + " Gyro: " + gyro.getAngle());
    	
    	aF.set((-1 * (magnetude + correction)) / highestValue);
    	aB.set((-1 * (magnetude - correction)) / highestValue);
    	bF.set((magnetude + correction) / highestValue);
    	bB.set((magnetude - correction) / highestValue);
    }
    
    public double getTicksForDistance(double feet)
    {
    	return 280 * feet / (4 * Math.PI);
    }
}